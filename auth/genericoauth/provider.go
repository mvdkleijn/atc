...
func (config *GenericOAuthConfig) AuthMethod(oauthBaseURL string) atc.AuthMethod {
...
func (GenericTeamProvider) AddAuthGroup(parser *flags.Parser) provider.AuthConfig {
	...
	group, err := parser.Group.AddGroup("Generic Auth", "Generic Authentication", flags)
	if err != nil {
		panic(err)
	}

	group.Namespace = "generic-oauth"
