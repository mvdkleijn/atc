...
type FakeAuthConfig struct {
	...
	AuthMethodStub        func(oauthBaseURL string) atc.AuthMethod
...
func (fake *FakeAuthConfig) AuthMethod(oauthBaseURL string) atc.AuthMethod {
	...
	fake.recordInvocation("AuthMethod", []interface{}{oauthBaseURL})
	fake.authMethodMutex.Unlock()
	if fake.AuthMethodStub != nil {
		return fake.AuthMethodStub(oauthBaseURL)
	}
	if specificReturn {
		return ret.result1
	}
...
func (fake *FakeAuthConfig) AuthMethodArgsForCall(i int) string {
	fake.authMethodMutex.RLock()
	defer fake.authMethodMutex.RUnlock()
	return fake.authMethodArgsForCall[i].oauthBaseURL
}
