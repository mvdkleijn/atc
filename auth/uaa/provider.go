...
func (*UAAAuthConfig) AuthMethod(oauthBaseURL string) atc.AuthMethod {
...
func (UAATeamProvider) AddAuthGroup(parser *flags.Parser) provider.AuthConfig {
	...
	group, err := parser.Group.AddGroup("UAA Auth", "UAA Authentication", flags)
	if err != nil {
		panic(err)
	}

	group.Namespace = "uaa-auth"
